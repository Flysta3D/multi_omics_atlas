...
def _cellpose(
    ...
    **kwargs,
    ...
    if isinstance(model, str):
        model = Cellpose(
            model_type=model, gpu=False, device=device
...
def segmentation_spateo(
    ...
    if seg_method in ["watershed", "w+s"]:
        ...
        if not (seg_kwargs is None):
            default_seg_kwargs.update(
                (k, seg_kwargs[k])
                for k in default_seg_kwargs.keys() & seg_kwargs.keys()
            )
        st.cs.mask_nuclei_from_stain(adata, otsu_index=default_seg_kwargs["otsu_index"])
        st.cs.find_peaks_from_mask(
            adata, layer="stain", min_distance=default_seg_kwargs["min_distance"]
        )
        st.cs.watershed(
            adata,
            layer="stain",
            k=default_seg_kwargs["k"],
            out_layer=expand_markers_layer,
        )
    if seg_method in ["stardist", "w+s"]:
        expand_markers_layer = "stardist_labels"
        st.cs.stardist(
            adata, layer="stain", out_layer=expand_markers_layer, **seg_kwargs
        )
    if seg_method == "w+s":
        expand_markers_layer = "augmented_labels"
        st.cs.augment_labels(
            adata, "watershed_labels", "stardist_labels", out_layer=expand_markers_layer
        )
    ...
    for label_type in check_label_types:
        ...
        contours = cv2.findContours(
            check_label.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )[0]
        if len(contours) != 1:
    ...
    if expand_method == "certain_distance":
        ...
        st.cs.expand_labels(
            adata, expand_markers_layer, out_layer="cell_labels", **expand_kwargs
        )
    ...
    if expand_distance != 0:
        st.cs.expand_labels(
            ...
            out_layer="cell_labels_expanded",
        )
...
def image_enhancement(
    ...
    local_thresh = threshold_local(
        image, block_size=block_size, method="gaussian", offset=offset
    )
